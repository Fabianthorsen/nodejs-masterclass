...
handlers._tokens.post = function (data, callback) {
  ...
  if (phone && password) {
    ...
    _data.read('users', phone, (err, userData) => {
      if (!err && userData) {
        // Hash sent password and compare to the password stored in user object
        const hashedPassword = helpers.hash(password);
        if (hashedPassword == userData.hashedPassword) {
          // If valid create new token with random name. Set expiration date 1hr in the future
          ...
          callback(400, {
            Error: 'Password did not matche the specified users password',
          });
        }
      } else {
        callback(400, { Error: 'Could not find specified user' });
...
handlers._tokens.put = function (data, callback) {
  ...
  if (id && extend) {
    ...
    _data.read('tokens', id, (err, tokenData) => {
      if (!err && tokenData) {
        // Check to make sure that token isnt expired
